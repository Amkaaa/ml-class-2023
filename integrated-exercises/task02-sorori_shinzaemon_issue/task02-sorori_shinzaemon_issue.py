...
def compute_sorori_shinzaemon(n_days = number_of_days):
    list_n_grains = [2**i for i in range(n_days)]
    list_total_grains = [sum(list_n_grains[:i+1]) for i in range(n_days)]
    return list_n_grains, list_total_grains

list_n_grains, list_total_grains = compute_sorori_shinzaemon(n_days = number_of_days)
plt.plot(range(1, number_of_days + 1), list_n_grains, label='Rice grains on the day')
plt.plot(range(1, number_of_days + 1), list_total_grains, label='Total rice grains by the day')
